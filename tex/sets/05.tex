\chapter{Ising model with the Metropolis algorithm}\label{ch:metropolis}

\section{Simulations at different temperatures}

\begin{sourcefiles}
  \item[Source code] src/051\_ising\_metropolis.c
  \item[Analysis script] src/051\_ising\_metropolis.R
  \item[Executable] exe/051\_ising\_metropolis
\end{sourcefiles}

For this and the following exercise, I implemented the Metropolis algorithm with
Glauber dynamics and sequential updating to simulate the two-dimensional Ising
model. The program outputs the total energy and magnetization after each sweep
of $N = L^{2}$ local spin flips.

In order to correctly estimate the observables of interest, after discarding an
appropriate number of equilibration time steps, I calculated the autocorrelation
time of each time series. Given an observable $\mathcal{O}$, we estimate its
expectation value $\expval{\mathcal{O}}$ using the Monte Carlo average
\begin{equation}
  \hat{\mathcal{O}} = \frac{1}{T} \sum_{t=1}^{T} \mathcal{O}_{t},
\end{equation}
where $T$ is the length of the time series after equilibration. The variance of
the estimator can be derived from the expression
\begin{align}
  \sigma_{\hat{\mathcal{O}}}^{2} &= \expval{\hat{\mathcal{O}}^{2}} -
  \expval{\hat{\mathcal{O}}}^{2} = \frac{1}{T^{2}} \sum_{s,t = 1}^{T}
  \expval{\mathcal{O}_{s} \mathcal{O}_{t}} - \frac{1}{T^{2}} \sum_{s,t = 1}^{T}
  \expval{\mathcal{O}_{s}} \expval{\mathcal{O}_{t}}. \label{eq:var-obar}
  \\
\intertext{If we collect the $s = t$ and $s \neq t$ terms, we get}
  \sigma_{\hat{\mathcal{O}}}^{2} &= \frac{1}{T^{2}} \sum_{t=1}^{T}
  \left(\expval{\mathcal{O}_{t}^{2}} - \expval{\mathcal{O}_{t}}^{2}\right) +
  \frac{1}{T^{2}} \sum_{s \neq t}^{T} \left(\expval{\mathcal{O}_{s}
  \mathcal{O}_{t}} - \expval{\mathcal{O}_{s}} \expval{\mathcal{O}_{t}}\right).
\end{align}
The first term is simply the variance of the time series,
$\sigma_{\mathcal{O}}^{2}$, divided by $T$. For the second term, we can rewrite
it as
\begin{align}
  \sum_{s \neq t}^{T} \left(\expval{\mathcal{O}_{s} \mathcal{O}_{t}} -
  \expval{\mathcal{O}_{s}} \expval{\mathcal{O}_{t}}\right) &= 2 \sum_{s = 1}^{T
  - 1} \sum_{t = 1}^{T - s} \left(\expval{\mathcal{O}_{s} \mathcal{O}_{t}} -
  \expval{\mathcal{O}_{s}} \expval{\mathcal{O}_{t}}\right), \\
\intertext{and then, since we are at equilibrium, we assume translational
invariance to group together all the pairs at distance $t$, of which there are
$T - t$:}
  \sum_{s \neq t}^{T} \left(\expval{\mathcal{O}_{s} \mathcal{O}_{t}} -
  \expval{\mathcal{O}_{s}} \expval{\mathcal{O}_{t}}\right) &=
  2 \sum_{t=1}^{T} \left(\expval{\mathcal{O}_{s} \mathcal{O}_{s+t}} -
  \expval{\mathcal{O}_{s}} \expval{\mathcal{O}_{s+t}}\right) (T - t),
\end{align}
where the $t = T$ cancels out. Inserting this result back into
\cref{eq:var-obar}, we can express:
\begin{equation}
  \sigma_{\hat{\mathcal{O}}}^{2} = \frac{\sigma_{\mathcal{O}}^{2}}{T} +
  \frac{2}{T} \sum_{t=1}^{T} \left(\expval{\mathcal{O}_{s}
  \mathcal{O}_{s+t}} - \expval{\mathcal{O}_{s}}
  \expval{\mathcal{O}_{s+t}}\right) \left(1 - \frac{t}{T}\right) 
  = \frac{\sigma_{\mathcal{O}}^{2}}{T} (1 + 2 \tau_{\mathcal{O}}),
  \label{eq:var-obar-final}
\end{equation}
where we define the \emph{integrated autocorrelation time} as
\begin{equation}
  \tau_{\mathcal{O}} \coloneq \sum_{t=1}^{T} \left(1 - \frac{t}{T}\right)
  C_{\mathcal{O}}(t),
  \label{eq:tau-def}
\end{equation}
with $C_{\mathcal{O}}$ being the \emph{autocorrelation function} defined as
\begin{equation}
  C_{\mathcal{O}}(t) \coloneq
  \frac{
    \expval{\mathcal{O}_{s} \mathcal{O}_{s + t}}
    - \expval{\mathcal{O}_{s}} \expval{\mathcal{O}_{s+t}}
  }{
    \expval{\mathcal{O}_{s}^{2}} - \expval{\mathcal{O}_{s}}^{2}
  }
\end{equation}

Although I used \cref{eq:tau-def} in my codes, one could simplify it knowing
that the autocorrelation function decays exponentially for $t \to \infty$:
\begin{equation}
  C_{\mathcal{O}}(t) \sim e^{-t / \tau_{\mathcal{O}}'},
\end{equation}
so that we can approximate the integrated correlation time with
\begin{equation}
  \tau_{\mathcal{O}}' \coloneq \sum_{t=1}^{T} C_{\mathcal{O}}(t) \approx
  \tau_{\mathcal{O}}.
\end{equation}

An additional consideration must be made when computing observables related to
second-order moments, such as specific heat and magnetic susceptibility. The
standard estimator for the variance is given by
\begin{equation}
  \hat{\sigma}_{\mathcal{O}}^{2} = \frac{1}{T} \sum_{t=1}^{T} (\mathcal{O}_{t} -
  \hat{\mathcal{O}})^{2}.
\end{equation}
In the presence of correlations, this estimator is biased.\footnote{It is biased
for uncorrelated samples too, but in that case it suffices to switch to the
sample variance $T \hat{\sigma}_{\mathcal{O}}^{2} / (T - 1)$.} Indeed, its
expected value is
\begin{align}
  \expval{\hat{\sigma}_{\mathcal{O}}^{2}} &= \expval[\bigg]{\frac{1}{T}
  \sum_{t=1}^{T} \mathcal{O}_{t}^{2}} - \expval{\hat{\mathcal{O}}^{2}}, \\
\intertext{and by adding and subtracting $\expval{\hat{\mathcal{O}}}^{2}$, we
obtain}
  \expval{\hat{\sigma}_{\mathcal{O}}^{2}} &= \expval[\bigg]{\frac{1}{T}
  \sum_{t=1}^{T} \mathcal{O}_{t}^{2}} - \expval{\hat{\mathcal{O}}}^{2} -
  \bigl(\expval{\hat{\mathcal{O}}^{2}} - \expval{\hat{\mathcal{O}}}^{2}\bigr).
  \\
\intertext{We can recognize the variance of the time series in the first two
terms, and the variance of the estimator $\hat{\mathcal{O}}$ in the second two.
Using the previously derived result from \cref{eq:var-obar-final}, we find}
  \expval{\hat{\sigma}_{\mathcal{O}}^{2}} &= \sigma_{\mathcal{O}}^{2} -
  \frac{\sigma_{\mathcal{O}}^{2}}{T} (1 + 2 \tau_{\mathcal{O}}) =
  \sigma_{\mathcal{O}}^{2} \left(1 - \frac{1 + 2 \tau_{\mathcal{O}}}{T}\right).
\end{align}
To adjust the estimations of $C_{V}$ and $\chi$ accordingly, I multiplied the
time series variance by $(T - 1) / (T - 1 - 2 \tau_{\mathcal{O}})$, with the
additional $-1$ at the numerator due to the use of the sample variance.
